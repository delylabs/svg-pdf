...
import type { FillRule, PreserveAspectRatioMode, ShapePaint, SvgInstruction } from '../types';
...
export function walkNode(
    ...
    if (SHAPE_ELEMENTS.has(tag)) {
        ...
        const needsBBox = [paint.fill, paint.stroke].some(
            (p) =>
                p !== null &&
                typeof p === 'object' &&
                'kind' in p &&
                p.kind === 'gradient' &&
                ctx.gradients.get(p.gradientId)?.gradientUnits !== 'userSpaceOnUse',
        );
